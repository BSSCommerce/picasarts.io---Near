...
export default function NftToken({media, title, owner_id, token_id, accountId, sale_conditions, bids, royalty, nearToUsd }) {
    ...
    >
        ...
        <CardHeader
            ...
            subheader={accountId !== owner_id ? <strong>by <NextLink key={`${token_id}_collection_link`} href={"/collection/[owner_id]"} as={`/collection/${owner_id}`} className={"nft-author-name"}>{formatAccountId(owner_id)}</NextLink></strong> : `You own this!`}
        />
